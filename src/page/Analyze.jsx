...
function Analyze() {
    ...
    // State to manage loading status and response data
    const [loading, setLoading] = useState(true);
    const {updateAnalyzeResult, getAnalyzeResult} = useAnalyzeResultStore();
    const navigate = useNavigate();

    // Destructure needed data from the store
    ...
    useEffect(() => {
        if (symptomSites.length === 0) {
            ...
            updateAnalyzeResult(response.data);
            console.log('Analysis result:', response.data);
            setLoading(false);
        }).catch((error) => {
            if(error.response.status === 400) {
                updateAnalyzeResult(defaultResult);
            } else {
...
return (
    <section className={body-font flex flex-col items-center h-full m-4 ${!loading && 'sm:w-1/3'}}>
        {/*<Maptest/>*/}
        {loading ? (<p></p>) : ((
            <div className="flex flex-col max-h-dvh">
                ...
                <div className="bg-gray-200 p-4 mt-4 rounded-lg border border-blue-500 shadow-lg">
                    <p className="text-blue-600 text-lg font-bold">{getAnalyzeResult.diseaseName}</p>
                    <p className="text-gray-700 mt-2">{getAnalyzeResult.diseaseSolution}</p>
                </div>
